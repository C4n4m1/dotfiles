...
Singleton {
    ...
    property bool activeToplevelMaximized: ToplevelManager.activeToplevel.maximized && (focusedWindowTitle != "")
    ...
    function updateFocusedWindow() {
        if (focusedWindowIndex >= 0 && focusedWindowIndex < windows.length) {
            ...
            // root.focusedWindowId = windows.find(w => w.is_focused)?.id;
            // root.focusedWindowTitle = windows.find(w => w.is_focused)?.title;
            focusedWindowTitle = "";
        }
    }
